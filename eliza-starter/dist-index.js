...
// src/actions/getPriceAction.ts
var priceTemplate = `# Task: Extract Cryptocurrency Information
...
function validateAndGetApiKey(runtime) {
  console.log("\u{1F6A8} VALIDATEANDGETAPIKEY FUNCTION CALLED - OUR CODE IS RUNNING!");
  let apiKey = runtime.getSetting("TOKENMETRICS_API_KEY");
  elizaLogger.log(`\u{1F510} Checking API key availability...`);
...
async function searchTokenByName(tokenName, runtime) {
  ...
  elizaLogger.log(`\u{1F50D} DEBUGGING: searchTokenByName called with: "${trimmedName}"`);
  try {
    const url = `https://api.tokenmetrics.com/v2/tokens?token_name=${encodeURIComponent(trimmedName)}`;
    elizaLogger.log(`\u{1F4E1} DEBUGGING: API URL: ${url}`);
    ...
    elizaLogger.log(`\u{1F4CA} DEBUGGING: searchTokenByName API Response:`, {
...
function mapSymbolToName(input) {
  ...
  if (mappedName) {
    elizaLogger.log(`\u{1F504} Mapped symbol "${input}" \u2192 "${mappedName}"`);
    return mappedName;
  }
  return input;
}
async function resolveTokenSmart(input, runtime) {
  elizaLogger.log(`\u{1F50D} DEBUGGING: resolveTokenSmart called with input: "${input}"`);
  elizaLogger.log(`\u{1F50D} DEBUGGING: Step 1 - Trying direct search for: "${input}"`);
  ...
  elizaLogger.log(`\u{1F50D} DEBUGGING: Step 2 - Symbol mapping: "${input}" \u2192 "${mappedName}"`);
  if (mappedName !== input) {
    elizaLogger.log(`\u{1F504} DEBUGGING: Trying mapped name: "${mappedName}"`);
...
async function fetchTokenMetricsPrice(tokenId, runtime) {
  try {
    const apiKey = validateAndGetApiKey(runtime);
    elizaLogger.log(`\u{1F4E1} DEBUGGING: fetchTokenMetricsPrice called for token_id: ${tokenId}`);
    const url = `https://api.tokenmetrics.com/v2/price?token_id=${tokenId}`;
    elizaLogger.log(`\u{1F310} DEBUGGING: Price API URL: ${url}`);
    const response = await fetchWithRetry(url, {
      method: "GET",
      headers: {
        "x-api-key": apiKey,
    ...
    elizaLogger.log(`\u{1F4C4} DEBUGGING: Price API Response length: ${rawData.length} characters`);
    let data;
    try {
    ...
    elizaLogger.log("\u{1F4CA} DEBUGGING: Price API Response structure:", {
      success: data.success,
      message: data.message,
      hasData: !!data.data,
...
function formatPriceResponse(priceData, tokenInfo) {
  ...
  let response = `\u{1F4B0} **${name} (${symbol})** is currently trading at **${formatCurrency(price)}**

`;
  if (priceData.PRICE_24H_CHANGE_PERCENT !== void 0 && priceData.PRICE_24H_CHANGE_PERCENT !== null) {
...
`;
  ...
  if (priceData.MARKET_CAP) {
    response += `\u{1F3DB}\uFE0F **Market Cap:** ${formatCurrency(priceData.MARKET_CAP)}
`;
  }
  if (priceData.VOLUME_24H) {
    response += `\u{1F4CA} **24h Volume:** ${formatCurrency(priceData.VOLUME_24H)}
`;
  }
  response += `
\u{1F517} *Real-time data from TokenMetrics API*`;
  return response;
}
function analyzePriceData(priceData) {
  const insights = [];
  const symbol = priceData.SYMBOL || priceData.TOKEN_SYMBOL || "this token";
  if (priceData.PRICE_24H_CHANGE_PERCENT !== void 0) {
...
var getPriceAction = {
  ...
  validate: async (runtime, message) => {
    elizaLogger.log("\u{1F50D} Validating TokenMetrics price action");
    try {
    ...
    const isValid2 = hasPriceKeyword && hasCryptoKeyword;
    elizaLogger.log(`\u2705 Validation result: ${isValid2} (price: ${hasPriceKeyword}, crypto: ${hasCryptoKeyword})`);
    return isValid2;
  },
...
Please try again in a few moments.`,
      ...
      elizaLogger.log(`\u{1F50D} DEBUGGING: Received price data for:`, {
...
// src/index.ts
elizaLogger2.log("\n=======================================");
elizaLogger2.log("   TokenMetrics Plugin Loading...     ");
elizaLogger2.log("=======================================");
elizaLogger2.log("Name      : tokenmetrics-plugin");
elizaLogger2.log("Version   : 2.1.0 (MINIMAL-INTERFACE)");
elizaLogger2.log("API Docs  : https://developers.tokenmetrics.com");
elizaLogger2.log("Real API  : https://api.tokenmetrics.com/v2");
elizaLogger2.log("");
elizaLogger2.log("\u{1F527} FEATURES IMPLEMENTED:");
elizaLogger2.log("\u2705 Natural Language Processing");
elizaLogger2.log("\u2705 Dynamic Token Resolution");
elizaLogger2.log("\u2705 Real TokenMetrics API Integration");
elizaLogger2.log("\u2705 Minimal TypeScript Interface Compliance");
elizaLogger2.log("\u2705 Zero TypeScript Errors");
elizaLogger2.log("\u2705 Self-Contained Architecture");
elizaLogger2.log("");
elizaLogger2.log("\u{1F3AF} PRICE ACTION CAPABILITIES:");
elizaLogger2.log("  \u2022 Understands natural language queries");
elizaLogger2.log("  \u2022 Dynamically discovers available tokens");
elizaLogger2.log("  \u2022 Provides real-time market data");
elizaLogger2.log("  \u2022 Offers intelligent market analysis");
elizaLogger2.log("=======================================\n");
var tokenmetricsPlugin = {
  ...
  actions: [
    getPriceAction
    // Our enhanced, self-contained price action
  ],
...
function validateTokenMetricsPlugin() {
  ...
  elizaLogger2.log("\u{1F50D} Validating TokenMetrics plugin configuration...");
  ...
  elizaLogger2.log(`\u{1F4CA} Plugin validation summary:`);
  elizaLogger2.log(`  \u2022 Actions: ${actions.length}`);
  elizaLogger2.log(`  \u2022 Evaluators: ${evaluators.length}`);
  elizaLogger2.log(`  \u2022 Providers: ${providers.length}`);
  elizaLogger2.log(`  \u2022 Services: ${services.length}`);
  if (isValid2) {
    elizaLogger2.log("\u2705 Plugin validation passed!");
  } else {
    elizaLogger2.error("\u274C Plugin validation failed:");
    issues.forEach((issue) => elizaLogger2.error(`  \u2022 ${issue}`));
  }
  if (recommendations.length > 0) {
    elizaLogger2.log("\u{1F4A1} Recommendations for improvement:");
    recommendations.forEach((rec) => elizaLogger2.log(`  \u2022 ${rec}`));
  }
  return { isValid: isValid2, issues, recommendations };
}
function debugTokenMetricsPlugin() {
  elizaLogger2.log("\u{1F9EA} TokenMetrics Plugin Debug Information:");
  elizaLogger2.log(`  \u{1F4CB} Plugin Name: ${tokenmetricsPlugin.name}`);
  elizaLogger2.log(`  \u{1F4CB} Description: ${tokenmetricsPlugin.description}`);
  const actions = tokenmetricsPlugin.actions || [];
  const evaluators = tokenmetricsPlugin.evaluators || [];
  const providers = tokenmetricsPlugin.providers || [];
  const services = tokenmetricsPlugin.services || [];
  elizaLogger2.log("  \u{1F527} Plugin Components:");
  elizaLogger2.log(`    \u2022 Actions: ${actions.length}`);
  elizaLogger2.log(`    \u2022 Evaluators: ${evaluators.length}`);
  elizaLogger2.log(`    \u2022 Providers: ${providers.length}`);
  elizaLogger2.log(`    \u2022 Services: ${services.length}`);
  if (actions.length > 0) {
    elizaLogger2.log("  \u{1F3AC} Available Actions:");
    actions.forEach((action, index) => {
      const similes = action.similes || [];
      const examples = action.examples || [];
      elizaLogger2.log(`    ${index + 1}. ${action.name}`);
      elizaLogger2.log(`       Description: ${action.description || "No description"}`);
      elizaLogger2.log(`       Similes: ${similes.length > 0 ? similes.join(", ") : "None"}`);
      elizaLogger2.log(`       Examples: ${examples.length}`);
    });
  }
}
function checkTokenMetricsEnvironment() {
  const missingVars = [];
  const suggestions = [];
  elizaLogger2.log("\u{1F50D} Checking TokenMetrics environment configuration...");
  ...
  if (!apiKeyFromEnv) {
    ...
    elizaLogger2.log("\u2705 TOKENMETRICS_API_KEY found in environment");
  ...
  if (isConfigured) {
    elizaLogger2.log("\u2705 TokenMetrics environment is properly configured!");
  } else {
    elizaLogger2.warn("\u26A0\uFE0F TokenMetrics environment configuration issues found:");
    missingVars.forEach((varName) => elizaLogger2.warn(`  \u2022 Missing: ${varName}`));
    if (suggestions.length > 0) {
      elizaLogger2.log("\u{1F4A1} Configuration suggestions:");
      suggestions.forEach((suggestion) => elizaLogger2.log(`  \u2022 ${suggestion}`));
    }
  }
  return { isConfigured, missingVars, suggestions };
}
function validatePluginRuntime() {
  elizaLogger2.log("\u{1F504} Performing runtime validation...");
  try {
    const actions = tokenmetricsPlugin.actions || [];
    if (actions.length === 0) {
      elizaLogger2.error("\u274C No actions available at runtime");
      return false;
    }
    for (const action of actions) {
      if (!action.name || typeof action.name !== "string") {
        elizaLogger2.error(`\u274C Action missing valid name`);
        return false;
      }
      if (typeof action.handler !== "function") {
        elizaLogger2.error(`\u274C Action ${action.name} handler is not a function`);
        return false;
      }
      if (typeof action.validate !== "function") {
        elizaLogger2.error(`\u274C Action ${action.name} validate is not a function`);
        return false;
      }
    }
    elizaLogger2.log("\u2705 Runtime validation passed!");
    elizaLogger2.log(`\u{1F4CA} Validated ${actions.length} actions successfully`);
    return true;
  } catch (error) {
    elizaLogger2.error("\u274C Runtime validation failed:", error);
    return false;
  }
}
elizaLogger2.log("\u{1F680} Running TokenMetrics plugin initialization checks...");
...
if (structureValidation.isValid && envValidation.isConfigured && runtimeValidation) {
  elizaLogger2.success("\u{1F389} TokenMetrics plugin fully initialized and ready!");
  elizaLogger2.log("\u{1F4AC} Users can now ask: 'What's the price of Bitcoin?'");
  elizaLogger2.log("\u{1F527} Plugin uses minimal interface - guaranteed TypeScript compatibility");
} else {
  elizaLogger2.warn("\u26A0\uFE0F TokenMetrics plugin loaded with some issues:");
  if (!structureValidation.isValid) elizaLogger2.warn("  \u2022 Plugin structure issues detected");
  if (!envValidation.isConfigured) elizaLogger2.warn("  \u2022 Environment configuration incomplete");
  if (!runtimeValidation) elizaLogger2.warn("  \u2022 Runtime validation failed");
  elizaLogger2.log("\u{1F4A1} Check the logs above for specific recommendations");
}
